...
func (bc *BlockChain) Add_Data(data string) error {
	...
	if err != nil {
		if errors.Is(err, &generalerrors.DataTooBig{Data: data, Data_Length: data_length}) {
			return err
		} else if errors.Is(err, &generalerrors.BlockCapacityReached{Capacity: block_data_capacity}) {
